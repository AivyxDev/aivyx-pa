...
use crate::app::{App, SettingsPopup, InputKind};
use crate::theme;
...
pub fn render(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    ly = render_card("LLM Provider", left_col.x, ly, left_col.width, 6, ci == 0, buf, |inner, buf| {
        let sel = |row: usize| if ci == 0 && app.settings_item_index == row { theme::highlight() } else { theme::text_bold() };
        kv(buf, inner, 0, "PROVIDER", &settings.provider_label, theme::text_bold());
        kv(buf, inner, 1, "MODEL", &settings.model_name, sel(0));
        let url_str = settings.provider_base_url.as_deref().unwrap_or("—");
        kv(buf, inner, 2, "URL", url_str, if ci == 0 && app.settings_item_index == 1 { theme::highlight() } else { theme::dim() });
        if let Some(ref embed) = settings.embedding_model {
            let dim_str = settings.embedding_dimensions.map(|d| format!(" ({d}d)")).unwrap_or_default();
            kv(buf, inner, 3, "EMBEDDING", &format!("{embed}{dim_str}"), theme::dim());
        }
    });

    // ── Autonomy (left, card 1) ──────────────────────────────
    ly = render_card("Autonomy", left_col.x, ly, left_col.width, 6, ci == 1, buf, |inner, buf| {
        let sel = |row: usize| if ci == 1 && app.settings_item_index == row { theme::highlight() } else { theme::text() };
        let tier_style = if ci == 1 && app.settings_item_index == 0 { theme::highlight() } else { theme::primary_bold() };
        ...
        kv(buf, inner, 1, "RATE LIMIT", &format!("{}/min", settings.max_tool_calls_per_min), sel(1));
        kv(buf, inner, 2, "MAX COST", &format!("${:.2}", settings.max_cost_usd), sel(2));
        let approval_str = if settings.require_approval_destructive { "Required for destructive" } else { "Not required" };
        kv(buf, inner, 3, "APPROVAL", approval_str, theme::text());
    });

    // ── Heartbeat (left, card 2) — interactive toggles ───────
    let hb_height = 14u16;
    ly = render_card("Heartbeat", left_col.x, ly, left_col.width, hb_height, ci == 2, buf, |inner, buf| {
        let status = if settings.heartbeat_enabled { "Enabled" } else { "Disabled" };
        let status_style = if settings.heartbeat_enabled { theme::sage() } else { theme::dim() };
        ...
        let flags: &[(&str, bool)] = &[
            ...
            ("extract knowledge", settings.heartbeat_can_extract_knowledge),
            ...
            ("notification pacing", settings.heartbeat_notification_pacing),
        ];
        ...
        for (i, (label, enabled)) in flags.iter().enumerate() {
            ...
            let marker_style = if *enabled { theme::sage() } else { theme::dim() };
            ...
            if i == 0 {
                ...
                let text_style = if is_selected { theme::highlight() } else { theme::text() };
    ...
    if !settings.schedules.is_empty() && ly + 4 < left_col.y + left_col.height {
        ...
        ly = render_card("Schedules", left_col.x, ly, left_col.width, height, ci == 3, buf, |inner, buf| {
            ...
            for (i, (name, cron_expr, enabled)) in settings.schedules.iter().enumerate() {
                ...
                let next_fire = if *enabled {
                    croner::Cron::new(cron_expr).parse().ok()
                ...
                let line = Line::from(vec![
                    ...
                    Span::styled(next_fire, if *enabled { theme::muted() } else { theme::dim() }),
                ]);
                buf.set_line(inner.x + 1, inner.y + i as u16, &line, inner.width - 2);
            }
        });
    ...
    // ── Agent (right, card 4) ────────────────────────────────
    ry = render_card("Agent", right_col.x, ry, right_col.width, 6, ci == 4, buf, |inner, buf| {
        let sel = |row: usize| if ci == 4 && app.settings_item_index == row { theme::highlight() } else { theme::text_bold() };
        kv(buf, inner, 0, "NAME", &settings.agent_name, sel(0));
        let soul_label = if settings.has_custom_soul { "custom" } else { "default" };
        kv(buf, inner, 1, "SOUL", soul_label, if ci == 4 && app.settings_item_index == 1 { theme::highlight() } else { theme::text() });
        let skills_str = if settings.agent_skills.is_empty() { "none".into() } else { settings.agent_skills.join(", ") };
        kv(buf, inner, 2, "SKILLS", &skills_str, if ci == 4 && app.settings_item_index == 2 { theme::highlight() } else { theme::dim() });
        kv(buf, inner, 3, "PERSONA", &settings.agent_persona, theme::dim());
    });

    // ── Integrations (right, card 5) ─────────────────────────
    let integrations = crate::app::App::integrations_list(settings);
    let integ_height = if ci == 5 { (integrations.len() as u16 + 2).min(15) } else { 6u16 };
    ry = render_card("Integrations", right_col.x, ry, right_col.width, integ_height, ci == 5, buf, |inner, buf| {
        if ci == 5 {
            ...
            for (i, (name, configured, _kind)) in integrations.iter().enumerate() {
                ...
                let marker_style = if *configured { theme::sage() } else { theme::dim() };
                let text_style = if is_sel { theme::highlight() } else { theme::text() };
                let line = Line::from(vec![
                    Span::styled(format!("{check} "), marker_style),
            ...
            for (name, configured, _kind) in &integrations {
                let (marker, style) = if *configured { ("●", theme::sage()) } else { ("○", theme::dim()) };
                ...
                buf.set_line(inner.x + 1 + col as u16, inner.y + row, &Line::from(vec![span]), (usable - col) as u16);
                col += item_width;
            }
        }
    });
    ...
    ry = render_card("Memory", right_col.x, ry, right_col.width, 5, ci == 6, buf, |inner, buf| {
        kv(buf, inner, 0, "MAX MEMORIES", &settings.max_memories.to_string(), theme::text());
        kv(buf, inner, 1, "SESSION AGE", &format!("{}h", settings.session_max_age_hours), theme::text());
        kv(buf, inner, 2, "GRAPH RECALL", &if settings.use_graph_recall { "Enabled" } else { "Disabled" }.to_string(), theme::text());
    });

    // ── Persona (right, card 7) ──────────────────────────────
    if let Some(ref persona) = settings.persona_dimensions {
        if ry + 6 < right_col.y + right_col.height {
            ry = render_card("Persona", right_col.x, ry, right_col.width, 7, ci == 7, buf, |inner, buf| {
                let dims = [
                ...
                for (i, (name, val)) in dims.iter().enumerate() {
                    ...
                    let name_style = if is_sel { theme::highlight() } else { theme::muted() };
                    let line = if is_sel {
                        Line::from(vec![
    ...
    if ry + 4 < right_col.y + right_col.height {
        ...
        ry = render_card("Tools & Extensions", right_col.x, ry, right_col.width, card_height, ci == 8, buf, |inner, buf| {
            kv(buf, inner, 0, "TOOLS", &format!("{} registered", app.tool_count), theme::text_bold());
            let disc_style = if ci == 8 && app.settings_item_index == 0 { theme::highlight() } else { theme::text() };
            ...
            if mcp_count == 0 {
                ...
                for (i, name) in settings.mcp_servers.iter().enumerate() {
                    let row = 2 + i;
                    if row as u16 >= inner.height { break; }
    ...
    if settings.desktop_configured && ly + 4 < left_col.y + left_col.height {
        ...
        ly = render_card("Applications", left_col.x, ly, left_col.width, card_height, ci == 9, buf, |inner, buf| {
            if app_count == 0 {
                let line = Line::from(Span::styled("  No apps detected. Enter to scan.", theme::dim()));
                buf.set_line(inner.x, inner.y, &line, inner.width);
            } else if ci == 9 {
                // Scrollable list with access level display
                let scroll_offset = app.settings_item_index.saturating_sub(inner.height.saturating_sub(1) as usize);
                for (i, (_bin, display_name, access)) in settings.desktop_app_access.iter().enumerate().skip(scroll_offset) {
                    ...
                    let line = if is_sel {
                        Line::from(vec![
                            Span::styled("◄ ", theme::primary()),
                            Span::styled(format!("{:<width$}", name_display, width = max_name), name_style),
                ...
                let counts: std::collections::HashMap<&str, usize> = settings.desktop_app_access.iter()
                    .fold(std::collections::HashMap::new(), |mut acc, (_, _, access)| {
    ...
    let help_text = if app.settings_popup.is_some() {
        match &app.settings_popup {
            ...
            Some(SettingsPopup::SkillManager { .. }) => "Enter add/done  d remove  ↑↓ select  Esc close",
            Some(SettingsPopup::IntegrationSetup { .. }) => "Tab next field  Enter save  Esc cancel",
...
fn render_card(
    ...
    let border_style = if selected { theme::primary() } else { theme::border() };
    let title_style = if selected { theme::primary_bold() } else { theme::text_bold() };
    let block = Block::default()
        .borders(Borders::ALL)
        .border_type(BorderType::Plain)
        .border_style(border_style)
        .title(Line::from(Span::styled(format!("[ {} ]", title.to_uppercase()), title_style)));
...
fn kv(buf: &mut Buffer, inner: Rect, row: u16, label: &str, value: &str, style: ratatui::style::Style) {
...
fn render_popup(popup: &SettingsPopup, frame_count: u64, area: Rect, buf: &mut Buffer) {
    ...
    match popup {
        SettingsPopup::TextInput { title, value, kind, .. } => {
            let rect = centered_rect(50, 5, area);
            Clear.render(rect, buf);
            let block = Block::default()
                .borders(Borders::ALL).border_type(BorderType::Plain)
                .border_style(theme::primary())
                .title(Line::from(Span::styled(format!("[ {} ]", title.to_uppercase()), theme::primary_bold())));
        ...
        SettingsPopup::MultiLineInput { title, lines, cursor_row, cursor_col, .. } => {
        ...
        SettingsPopup::SkillManager { input, selected, skills } => {
            ...
            let block = Block::default()
                .borders(Borders::ALL).border_type(BorderType::Plain)
                .border_style(theme::primary())
                .title(Line::from(Span::styled("[ SKILLS ]", theme::primary_bold())));
            let inner = block.inner(rect);
            block.render(rect, buf);
            ...
            for (i, skill) in skills.iter().enumerate() {
                if (i + 1) as u16 >= inner.height { break; }
                let is_sel = *selected == i;
                let marker = if is_sel { ">" } else { " " };
                let style = if is_sel { theme::highlight() } else { theme::text() };
                let line = Line::from(vec![
        ...
        SettingsPopup::IntegrationSetup { fields, focused, is_configured, .. } => {
            let h = (fields.len() as u16 + 3).min(14);
            let rect = centered_rect(55, h, area);
            Clear.render(rect, buf);
            let title_text = if *is_configured { "[ RECONFIGURE ]" } else { "[ SETUP INTEGRATION ]" };
            let block = Block::default()
                .borders(Borders::ALL).border_type(BorderType::Plain)
                .border_style(theme::primary())
            ...
            for (i, field) in fields.iter().enumerate() {
                ...
                let label_style = if is_focused { theme::highlight() } else { theme::muted() };
                let val_display = if field.is_secret {
                    // Mask secret fields with dots
                    if field.value.is_empty() {
                        if is_focused { cursor_char.to_string() } else { "(enter to set)".into() }
                    } else if is_focused {
